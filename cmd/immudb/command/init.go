...
func (cl *Commandline) setupFlags(cmd *cobra.Command, options *server.Options) {
	...
	cmd.Flags().Bool("az-blob-storage", false, "Enable or disable Azure Storage Blob storage")
	cmd.Flags().String("az-blob-endpoint", "", "Azure Storage Blob endpoint (e.g., 'https://foostorageaccount.blob.core.windows.net/somecontainer')")
	cmd.Flags().String("az-blob-container", "", "The Azure storage container name (e.g., 'somecontainer')")
	cmd.Flags().String("az-blob-prefix", "", "The Azure blob prefix")
...
func setupDefaults(options *server.Options) {
	...
	viper.SetDefault("az-blob-storage", false)
	viper.SetDefault("az-blob-endpoint", "")
	viper.SetDefault("az-blob-container", "")
	viper.SetDefault("az-blob-prefix", "")
